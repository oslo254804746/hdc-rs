...
#[tokio::test]
#[ignore] // Requires HDC server and device
async fn test_reverse_forward_parity_commands() {
    init_logger();
    ...
    let tasks = client.rport_list().await;
    info!("Reverse forward tasks: {:?}", tasks);

    let result = client.rport_remove("tcp:19090 tcp:19091").await;
    info!("Reverse forward remove result: {:?}", result);
}

#[tokio::test]
#[ignore] // Requires HDC server, device, and valid sideload package path
async fn test_sideload_command() {
    init_logger();
    ...
    let result = client.sideload("/path/to/update.zip").await;
    info!("Sideload result: {:?}", result);
}
